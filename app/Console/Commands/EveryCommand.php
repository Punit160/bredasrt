...
class EveryCommand extends Command
{
    /**
     ...
     * @var string
     */
     protected $signature = 'everycommand:update';
   ...
   public function handle()
    {
        ...
        $single = DB::table('notifications')->where('task_type', '2')->where('frequency', '1')->get();
        foreach($single as $singles){
          $userdetail =  DB::table('notifications')->where('id', $singles->id)->whereDate('before_date','<=',$date)->where('deadline_time', $time)->first();
          if($userdetail){
