...
use App\Models\KsbComplaint;
use Session;
use Mail;

class KsbComplaintController extends Controller
{
    public function add_complaint(Request $request)
    {
        $data['state'] = $request->session()->get('state');
        $data['district'] = DB::table('ksbengineers')->select('district')->get();
    ...
    public function create(Request $request)
    {
        ...
        $eng = DB::table('ksbengineers')->where('district', $request->district)->first();
        $complaint = new KsbComplaint();
    ...
    public function view_complaint(Request $request)
    {
        ...
        $data['data'] = DB::table('ksbcomplaints')->paginate(50);
    ...
    public function view_eng_complaint(Request $request)
    {
        ...
        $eng = DB::table('ksbengineers')->where('contact', $userdetail->mobile)->first();
        $data['data'] = DB::table('ksbcomplaints')->where('engineer_assign', $eng->id)->paginate(50);
       ...
       public function final_complaint(Request $request)
    {
        ...
        $data['loginrole'] = $request->session()->get('role');
        $data['state'] = $request->session()->get('state');
        $data['data'] = DB::table('ksbcomplaints')->where('solved', '!=' , '')->paginate(50);
    ...
    {
        $data['complaint'] = KsbComplaint::find($id);
        $data['state'] = $request->session()->get('state');
    ...
    { 
        $id = $request->id;
        $Complaint = KsbComplaint::find($id);
        $eng = DB::table('ksbengineers')->where('id', $Complaint->engineer_assign)->first();
        $created_by = $request->session()->get('login');
        ...
        return redirect(route('view-kusumb-complaint'))->with('status', 'Complaint Verified Successfully');
    }
    ...
    { 
       ...
       $eng = DB::table('ksbengineers')->where('id', $Complaint->engineer_assign)->first();
        ...
        return redirect(route('view-kusumb-complaint'))->with('status', 'Complaint Closed Successfully');
    }
